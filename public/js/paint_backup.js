...
const paintAppSkipInit = Boolean(window.paintApp.__initialized);
window.paintApp.__initialized = true;

if (paintAppSkipInit) {
  console.warn('[PAINT.JS] Skipping duplicate initialization');
}
...
if (!paintAppSkipInit) {
  document.addEventListener('DOMContentLoaded', () => {
    if (window.paintApp.state.uiInitialized) return;
    window.paintApp.state.uiInitialized = true;
    ...
    if (quickSaveBtn && saveButton && !quickSaveBtn.__quickSaveBound) {
      quickSaveBtn.__quickSaveBound = true;
      quickSaveBtn.addEventListener('click', event => {
        event.preventDefault();
        event.stopPropagation();
        if (saveButton.__quickSaveInFlight) return;
    ...
    // Helper function to clone ImageData (needed early for undo stack initialization)
    ...
    function addImageToSidebar(imageUrl, label, filename) {
      ...
      if (typeof window.observeImageContainer === 'function')
        window.observeImageContainer(container);
    ...
    const originalUpdateStrokeVisibilityControls =
      typeof window.updateStrokeVisibilityControls === 'function'
    ...
    // IIFE for canvas event handlers and additional initialization
    // DOM elements already initialized above, just create local references
    (() => {
      const canvas = window.paintApp.state.domElements.canvas;
      const ctx = window.paintApp.state.domElements.ctx;
      const colorPicker = window.paintApp.state.domElements.colorPicker;
      ...
      function updateStrokeCounter() {
        const strokeCount =
          window.paintApp.state.lineStrokesByImage[window.paintApp.state.currentImageLabel]
            ?.length || 0;
        strokeCounter.textContent = `Lines: ${strokeCount}`;
      ...
      function saveAllImages() {
        ...
        // Check multiple possible storage locations for images
        // 1. Legacy system: window.originalImages or window.paintApp.state.originalImages
        // 2. New Fabric.js system: window.imageGalleryData
        const originalImages = window.originalImages || window.paintApp.state.originalImages || {};
        let originalImageLabels = Object.keys(originalImages);

        // If no legacy images, try the new gallery system
        if (
          originalImageLabels.length === 0 &&
          window.imageGalleryData &&
          window.imageGalleryData.length > 0
        ) {
          // Build image labels from gallery data
          const labelsFromGallery = [];
          const processedLabels = new Set();

          window.imageGalleryData.forEach(item => {
            const label = item.original?.label || item.label || item.name;
            if (label && !processedLabels.has(label)) {
              processedLabels.add(label);
              labelsFromGallery.push(label);
              // Also populate originalImages for downstream functions
              if (item.src || item.url) {
                originalImages[label] = item.src || item.url;
              }
            }
          });

          originalImageLabels = labelsFromGallery;
          // Store back to window for other functions to use
          window.originalImages = originalImages;
        }
      ...
      window.saveSelectedImages = function (projectName) {
        ...
        const allRadios = document.querySelectorAll(
          '#imageSelectionOverlay input[name="exportMode"]'
        );
      ...
      window.showPDFExportDialog = async function (projectName) {
        // Hide floating controls while dialog is open
        ...
        for (const imageLabel in window.vectorStrokesByImage) {
          ...
          strokeOrder.forEach(strokeLabel => {
            ...
            if (vectorData && strokeLabel && strokeLabel.trim()) {
              // Get measurement from window.strokeMeasurements (new Fabric.js system)
              // or from vectorData.measurement (legacy system)
              let measurementValue = 'N/A';
              const storedMeasurement = window.strokeMeasurements?.[imageLabel]?.[strokeLabel];
              if (storedMeasurement) {
                // New system - measurement is stored separately
                const unit = window.currentUnit || 'inch';
                if (unit === 'inch') {
                  const whole = storedMeasurement.inchWhole || 0;
                  const fraction = storedMeasurement.inchFraction || 0;
                  if (whole > 0 || fraction > 0) {
                    if (fraction === 0) {
                      measurementValue = `${whole}"`;
                    } else {
                      const fractionMap = {
                        0.125: '1/8',
                        0.25: '1/4',
                      ...
                      const fractionStr = fractionMap[fraction] || fraction.toFixed(3);
                      measurementValue = whole > 0 ? `${whole} ${fractionStr}"` : `${fractionStr}"`;
                    }
                  }
                } else {
                  const cm = storedMeasurement.cm || 0;
                  if (cm > 0) {
                    measurementValue = `${cm.toFixed(1)} cm`;
                  }
                }
              } else if (vectorData.measurement) {
                // Legacy system - measurement stored on object
                measurementValue = vectorData.measurement;
              }

              // Get color from Fabric object or legacy data
              const color = vectorData.stroke || vectorData.color || '#000000';
        ...
        dialogOverlay.innerHTML = `
            <div style="background: white; border-radius: 8px; width: 90%; max-width: 800px; max-height: 90vh; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.3);">
                ...
                <div style="padding: 20px; overflow-y: auto; max-height: calc(90vh - 160px);">
                    ...
                    <div id="framesSection" style="margin-bottom: 20px; padding: 15px; background: #fff3e0; border-radius: 4px; border-left: 4px solid #ff9800;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                            <div>
                                <label style="display: flex; align-items: center; cursor: pointer;">
                                    <input type="checkbox" id="useFramesForPDF" style="margin-right: 8px;">
                                    <span style="font-weight: bold; color: #333;">Use frames for PDF generation</span>
                                </label>
                                <p style="margin: 5px 0 0 24px; font-size: 12px; color: #666;">
                                    Each frame becomes a separate PDF page showing only the cropped area and its measurements.
                                </p>
                            </div>
                            <button id="defineFramesBtn" style="padding: 8px 16px; border: 1px solid #ff9800; border-radius: 4px; background: white; color: #ff9800; cursor: pointer; font-weight: 500;">
                                Define Frames
                            </button>
                        </div>
                        <div id="framesList" style="display: none; max-height: 200px; overflow-y: auto; border: 1px solid #ddd; border-radius: 4px; background: white;">
                            <div id="framesListContent" style="padding: 10px;">
                                <p style="color: #666; font-style: italic; text-align: center;">No frames defined yet. Click "Define Frames" to draw frame areas on your images.</p>
                            </div>
                        </div>
                    </div>
        ...
        // Restore canvasControls when dialog is closed (via any method)
        const observer = new MutationObserver(mutations => {
          mutations.forEach(mutation => {
            mutation.removedNodes.forEach(node => {
              if (node === dialogOverlay || node.id === 'pdfExportOverlay') {
        ...
        // Frames section event listeners
        const useFramesCheckbox = document.getElementById('useFramesForPDF');
        const defineFramesBtn = document.getElementById('defineFramesBtn');
        const framesList = document.getElementById('framesList');

        // Update frames list display
        function updateFramesList() {
          const framesListContent = document.getElementById('framesListContent');
          if (!framesListContent) return;

          const allFrames = window.pdfFramesByImage || {};
          let totalFrames = 0;
          let html = '';

          for (const imageLabel in allFrames) {
            const frames = allFrames[imageLabel] || [];
            if (frames.length > 0) {
              html += `<div style="margin-bottom: 10px;">
                        <strong style="color: #333;">${imageLabel}</strong>
              ...
              frames.forEach((frame, index) => {
                ...
                html += `<div style="display: flex; align-items: center; justify-content: space-between; padding: 5px; border-bottom: 1px solid #eee;">
                            <span style="font-size: 13px;">${frame.name} (${Math.round(frame.bounds.width)}x${Math.round(frame.bounds.height)})</span>
                            <button onclick="window.deleteFrame('${imageLabel}', '${frame.id}')" style="padding: 2px 8px; border: 1px solid #f44336; border-radius: 3px; background: white; color: #f44336; cursor: pointer; font-size: 11px;">Delete</button>
                        </div>`;
              });
          ...
          if (totalFrames === 0) {
            ...
            html =
              `<p style="margin: 0 0 10px 0; font-weight: bold; color: #333;">Total: ${totalFrames} frame(s) = ${totalFrames} PDF page(s)</p>` +
              html;
          }

          framesListContent.innerHTML = html;
        }

        // Toggle frames list visibility based on checkbox
        useFramesCheckbox.addEventListener('change', function () {
          framesList.style.display = this.checked ? 'block' : 'none';
          updateFramesList();
        });

        // Define Frames button - activate frame tool
        defineFramesBtn.addEventListener('click', function () {
          // Close the dialog temporarily
          dialogOverlay.style.display = 'none';

          // Activate frame tool
          if (window.app && window.app.toolManager) {
            window.app.toolManager.selectTool('frame');

            // Show instructions
            const instructions = document.createElement('div');
            instructions.id = 'frameToolInstructions';
            ...
            instructions.innerHTML = `
                    <span>Draw rectangles to define PDF frame areas. Click and drag to create frames.</span>
                    <button onclick="window.finishDefiningFrames()" style="padding: 6px 12px; border: none; border-radius: 4px; background: white; color: #ff9800; cursor: pointer; font-weight: bold;">Done</button>
                `;
            document.body.appendChild(instructions);
          }
        });

        // Global function to finish defining frames
        window.finishDefiningFrames = function () {
          // Remove instructions
          const instructions = document.getElementById('frameToolInstructions');
          ...
          // Check the "use frames" checkbox
          useFramesCheckbox.checked = true;
        ...
        // Global function to delete a frame
        window.deleteFrame = function (imageLabel, frameId) {
          if (window.pdfFramesByImage && window.pdfFramesByImage[imageLabel]) {
        ...
        window.addEventListener('frameCreated', updateFramesList);
        window.addEventListener('frameUpdated', updateFramesList);
        window.addEventListener('frameDeleted', updateFramesList);
      ...
      window.generatePDF = async function (projectName) {
        try {
          ...
          // Check if frame-based PDF generation is enabled
          const useFrames = document.getElementById('useFramesForPDF')?.checked || false;

          if (useFrames) {
            // Frame-based PDF generation
            await generateFrameBasedPDF(pdf, projectName, selectedUnit);
            return;
          }
          ...
          for (const container of imageContainers) {
            ...
            // Debug logging
            console.log(
              `[PDF Debug] Image: ${imageLabel}, strokeMeasurements:`,
              strokeMeasurements
            );
          ...
          if (
            originalImageLabel &&
            originalImageLabel !== window.paintApp.state.currentImageLabel
          ) {
      ...
      // Frame-based PDF generation function
      async function generateFrameBasedPDF(pdf, projectName, selectedUnit) {
        const canvas = document.getElementById('canvas');
        const originalImageLabel =
          window.paintApp?.state?.currentImageLabel || window.currentImageLabel;
        const allFrames = window.pdfFramesByImage || {};

        // Collect all frames across all images
        ...
        if (framesToProcess.length === 0) {
          alert('No frames defined. Please define frames before generating a frame-based PDF.');
          return;
        }

        console.log(`[PDF Frames] Generating PDF with ${framesToProcess.length} frames`);

        let isFirstPage = true;

        for (let i = 0; i < framesToProcess.length; i++) {
          const { imageLabel, frame } = framesToProcess[i];

          if (!isFirstPage) {
            pdf.addPage();
          }

          // Switch to the target image if needed
          ...
          const imageName =
            typeof window.getUserFacingImageName === 'function'
              ...
              : imageLabel;
          pdf.text(`${projectName} - ${imageName}`, 15, yPosition);
          yPosition += 6;

          pdf.setFontSize(12);
          pdf.setFont(undefined, 'normal');
          pdf.setTextColor(100, 100, 100);
          pdf.text(frame.name || `Frame ${i + 1}`, 15, yPosition);
          pdf.setTextColor(0, 0, 0);
          yPosition += 4;
          ...
          // Capture the cropped region of the canvas
          try {
            const croppedImage = await captureFrameRegion(canvas, frame.bounds);

            if (croppedImage) {
              // Calculate dimensions to fit on page (max width 180mm, max height 140mm)
              const maxWidth = 180;
              const maxHeight = 140;
              const aspectRatio = frame.bounds.width / frame.bounds.height;
              ...
              // Center the image
              const imgX = (210 - imgWidth) / 2;
              pdf.addImage(croppedImage, 'JPEG', imgX, yPosition, imgWidth, imgHeight);
              yPosition += imgHeight + 10;
            }
          } catch (error) {
            console.warn(`Could not capture frame ${frame.name}:`, error);
            pdf.setFontSize(12);
            pdf.setFont(undefined, 'italic');
            pdf.text(`[Frame image could not be captured]`, 15, yPosition);
            yPosition += 15;
          }

          // Find measurements within this frame's bounds
          const measurementsInFrame = getMeasurementsInFrame(imageLabel, frame.bounds);

          if (measurementsInFrame.length > 0) {
            ...
            // Table header
            const tableX = 15;
            const colWidths = [50, 45, 45]; // Label, Inches, CM
            const rowHeight = 7;
            ...
            pdf.setFontSize(9);
            pdf.setFont(undefined, 'bold');
            pdf.text('Label', tableX + 3, yPosition);
            pdf.text('Inches', tableX + colWidths[0] + 3, yPosition);
            pdf.text('CM', tableX + colWidths[0] + colWidths[1] + 3, yPosition);

            // Draw vertical lines for header
            pdf.line(tableX + colWidths[0], yPosition - 4, tableX + colWidths[0], yPosition + 3);
            pdf.line(
            ...
            // Draw data rows
            pdf.setFont(undefined, 'normal');
            for (const measurement of measurementsInFrame) {
              ...
              if (yPosition > 270) {
                ...
                pdf.setFontSize(11);
                pdf.setFont(undefined, 'bold');
                pdf.text('MEASUREMENTS (continued)', 15, yPosition);
                yPosition += 8;
                pdf.setFont(undefined, 'normal');
                pdf.setFontSize(9);
              }
              ...
              // Draw cell content
              pdf.text(measurement.label, tableX + 3, yPosition);
              pdf.text(measurement.inches, tableX + colWidths[0] + 3, yPosition);
              pdf.text(measurement.cm, tableX + colWidths[0] + colWidths[1] + 3, yPosition);
            ...
            pdf.setFont(undefined, 'italic');
            pdf.setTextColor(100, 100, 100);
            pdf.text('No measurements in this frame area.', 15, yPosition);
            pdf.setTextColor(0, 0, 0);
            yPosition += 10;
          }
          ...
          if (yPosition < 275) {
            ...
            pdf.text(`Page ${i + 1} of ${framesToProcess.length}`, 190, 290, { align: 'right' });
            pdf.setTextColor(0, 0, 0);
          }
        ...
        // Save the PDF
        const fileName = `${projectName.replace(/[^a-z0-9]/gi, '_')}_frames_${new Date().toISOString().split('T')[0]}.pdf`;
        pdf.save(fileName);
        ...
        if (typeof window.projectManager?.showStatusMessage === 'function') {
          window.projectManager.showStatusMessage(
            `Frame-based PDF "${fileName}" generated successfully!`,
            'success'
          );
        } else {
          alert(`Frame-based PDF "${fileName}" generated successfully!`);
        }
      }

      // Capture a cropped region of the canvas as a data URL
      async function captureFrameRegion(canvas, bounds) {
        return new Promise(resolve => {
          try {
            const tempCanvas = document.createElement('canvas');
            const tempCtx = tempCanvas.getContext('2d');
            const scale = 2; // High resolution capture
            ...
            console.error('Error capturing frame region:', error);
            resolve(null);
          }
        });
      }

      // Find measurements whose stroke endpoints fall within frame bounds
      function getMeasurementsInFrame(imageLabel, bounds) {
        const results = [];
        const strokes = window.vectorStrokesByImage?.[imageLabel] || {};
        const measurements = window.strokeMeasurements?.[imageLabel] || {};

        for (const strokeLabel in strokes) {
          const stroke = strokes[strokeLabel];
          const measurement = measurements[strokeLabel];

          if (!stroke || !stroke.points || stroke.points.length < 2) continue;

          // Check if stroke endpoints are within bounds
          const points = stroke.points;
          const startPoint = points[0];
          const endPoint = points[points.length - 1];

          const startInBounds = isPointInBounds(startPoint, bounds);
          const endInBounds = isPointInBounds(endPoint, bounds);

          // Include if at least one endpoint is in bounds
          if (startInBounds || endInBounds) {
            let inchValue = 'N/A';
            let cmValue = 'N/A';

            if (measurement) {
              if (measurement.inchWhole !== undefined) {
                const inches = measurement.inchWhole + (measurement.inchFraction || 0);
                inchValue = formatInches(inches);
              }
              if (measurement.cm !== undefined) {
                cmValue = `${measurement.cm.toFixed(1)} cm`;
              }
            }
      ...
      // Format inches as fractional string (e.g., 5 1/2")
      function formatInches(inches) {
        ...
        if (whole === 0) {
          return `${closestFrac.display}"`;
        }

        return `${whole} ${closestFrac.display}"`;
      }
          ...
          for (let i = 0; i < selectedLabels.length; i++) {
            ...
            try {
              ...
              if (window.getTagBasedFilename && typeof window.getTagBasedFilename === 'function') {
                const tagBasedName = window.getTagBasedFilename(
                  imageLabel,
                  imageLabel.split('_')[0]
                );
      ...
      async function processImagesSequentiallyForScreenView(
        ...
        for (let i = 0; i < selectedLabels.length; i++) {
          ...
          console.log(
            `[${modeLabel}] Processing ${imageLabel} (${i + 1}/${selectedLabels.length})`
          );
          ...
          const blob = await new Promise(resolve =>
            tempCanvas.toBlob(resolve, fileFormat, quality)
          );
        ...
        if (
          originalImageLabel &&
          originalImageLabel !== selectedLabels[selectedLabels.length - 1]
        ) {
          window.paintApp.state.currentImageLabel = originalImageLabel;
      ...
      function createStrokeVisibilityControl(strokeLabel, context) {
        ...
        const isMultiSelected =
          multipleSelectedStrokesByImage[currentImageLabel].includes(strokeLabel);
        ...
        const measureTextElement = createEditableMeasureText(
          strokeLabel,
          isSelected,
          labelContainer
        );
      ...
      function updateStrokeVisibilityControls() {
        ...
        if (currentSelectionArray.length === 1) {
          ...
          if (!currentSelectionArray.includes(selectedStrokeByImage[currentImageLabel])) {
            console.warn(
              `[updateStrokeVisibilityControls] Correcting selectedStrokeByImage for multi-select. Was: ${selectedStrokeByImage[currentImageLabel]}, multiple was: ${JSON.stringify(currentSelectionArray)}. Setting to: ${currentSelectionArray[0] || null}`
            );
          ...
          if (selectedStrokeByImage[currentImageLabel] !== null) {
            console.warn(
              `[updateStrokeVisibilityControls] Correcting selectedStrokeByImage. Was: ${selectedStrokeByImage[currentImageLabel]}, multiple was empty. Setting to: null`
            );
        ...
        if (isArrowLine && vectorData.points.length >= 2) {
          ...
          if (vectorData.arrowSettings) {
            ...
            const baseArrowSize = Math.max(
              vectorData.arrowSettings.arrowSize || 15,
      ...
      function drawStrokeDecorations(
        ...
        if (isCurvedArrow && vectorData.arrowSettings && vectorData.points.length >= 2) {
          ...
          if (vectorData.points.length >= 2) {
            ...
            const startTransformed = imageToCanvasCoords(
              firstPoint.x,
              firstPoint.y,
              transformParams
            );
      ...
      function applyVisibleStrokes(scale, imageX, imageY) {
        ...
        if (!vectorStrokesByImage[currentImageLabel]) {
          console.warn(
            `[applyVisibleStrokes] vectorStrokesByImage[${currentImageLabel}] was undefined, initializing...`
          );
          vectorStrokesByImage[currentImageLabel] = {};
        }
        ...
        strokeOrder.forEach(strokeLabel => {
          ...
          // Check if this is a Fabric.js object (new system) - skip legacy drawing
          // Fabric objects have 'type' and methods like 'toObject' but no 'points' array
          if (vectorData.type && typeof vectorData.toObject === 'function') {
            // This is a Fabric.js object, it's already rendered by Fabric canvas
            return;
          }
          if (!vectorData.points || vectorData.points.length === 0) {
            // Only warn for legacy format strokes that are malformed
            if (!vectorData.type || typeof vectorData.toObject !== 'function') {
        ...
        strokeOrder.forEach(strokeLabel => {
          ...
          // Skip Fabric.js objects (new system) - labels are handled by Fabric
          if (vectorData && vectorData.type && typeof vectorData.toObject === 'function') {
            return; // This is a Fabric object, skip legacy label drawing
          }
          ...
          if (
            ...
            if (vectorData.points.length > 0) {
              ...
              try {
                ...
                if (
                  !anchorPointCanvas ||
                  isNaN(anchorPointCanvas.x) ||
                  isNaN(anchorPointCanvas.y)
                ) {
                  console.error(
            ...
            console.log(
              `[OFFSET-DEBUG] ${strokeLabel} - customLabelPositions exists:`,
              !!customLabelPositions[currentImageLabel]?.[strokeLabel]
            );
            console.log(
              `[OFFSET-DEBUG] ${strokeLabel} - calculatedLabelOffsets exists:`,
              !!calculatedLabelOffsets[currentImageLabel]?.[strokeLabel]
            );
            ...
            const windowCustomExists =
              window.customLabelPositions[currentImageLabel]?.[strokeLabel];

            if (localCustomExists && !windowCustomExists) {
              console.log(
                `[SYNC-WARNING] ${strokeLabel} - Found in local but not window storage - this may cause rotation issues`
              );
            } else if (!localCustomExists && windowCustomExists) {
              console.log(
                `[SYNC-WARNING] ${strokeLabel} - Found in window but not local storage - syncing to local`
              );
            ...
            if (calculatedLabelOffsets[currentImageLabel]?.[strokeLabel]) {
              console.log(
                `[OFFSET-DEBUG] ${strokeLabel} - calculated offset value:`,
                calculatedLabelOffsets[currentImageLabel][strokeLabel]
              );
            }
            if (customLabelPositions[currentImageLabel]?.[strokeLabel]) {
              console.log(
                `[OFFSET-DEBUG] ${strokeLabel} - custom position value:`,
                customLabelPositions[currentImageLabel][strokeLabel]
              );
            }
            ...
            if (relativePos) {
              ...
              if (absFromRelative) {
                ...
                console.log(
                  `[OFFSET-DEBUG] ${strokeLabel} - USING RELATIVE (derived absolute):`,
                  imageSpaceOffset
                );
              }
            }
            if (!imageSpaceOffset && customLabelPositions[currentImageLabel]?.[strokeLabel]) {
              ...
              if (currentImageLabel === 'blank_canvas') {
                ...
                if (offsetMagnitude > 300 && !window.clearedMassiveOffsets[offsetKey]) {
                  console.log(
                    `[Label] Clearing massive offset for blank canvas ${strokeLabel}: (${imageSpaceOffset.x.toFixed(1)}, ${imageSpaceOffset.y.toFixed(1)}) magnitude: ${offsetMagnitude.toFixed(1)}`
                  );
                  ...
                  console.log(
                    `[Label] Preserving reasonable offset for blank canvas ${strokeLabel}: (${imageSpaceOffset.x.toFixed(1)}, ${imageSpaceOffset.y.toFixed(1)}) magnitude: ${offsetMagnitude.toFixed(1)}`
                  );
                }
              }
            ...
            if (!imageSpaceOffset) {
              ...
              if (typeof findOptimalLabelPosition !== 'function') {
                ...
                const fallbackLabelTopLeftImg = getTransformedCoords(
                  fallbackCanvasX,
                  fallbackCanvasY
                );
                imageSpaceOffset = {
            ...
            console.log(
              `[OFFSET-DEBUG] ${strokeLabel} - FINAL OFFSET BEING USED:`,
              imageSpaceOffset
            );
            ...
            console.log(
              `[Label] Final position for ${strokeLabel}: Image(${finalLabelImageX.toFixed(1)}, ${finalLabelImageY.toFixed(1)}) -> Canvas(${finalPositionCanvas.x.toFixed(1)}, ${finalPositionCanvas.y.toFixed(1)}) | Anchor: Image(${anchorPointImage.x.toFixed(1)}, ${anchorPointImage.y.toFixed(1)}) + Offset: (${imageSpaceOffset.x.toFixed(1)}, ${imageSpaceOffset.y.toFixed(1)})`
            );
            ...
            if (lastAction.deletedStrokeLabels) {
              lastAction.deletedStrokeLabels.forEach(strokeLabel => {
                ...
                strokeLabelVisibility[lastAction.image][strokeLabel] =
                  lastAction.labelVisibility &&
                  lastAction.labelVisibility[strokeLabel] !== undefined
        ...
        if (redoStack && redoStack.length > 0) {
          ...
          // Handle stroke type actions (freehand strokes, straight lines, and curved lines)
            ...
            if (actionToRedo.labelVisible !== undefined) {
              strokeLabelVisibility[currentImageLabel] =
                strokeLabelVisibility[currentImageLabel] || {};
              strokeLabelVisibility[currentImageLabel][actionToRedo.label] =
                actionToRedo.labelVisible;
            } else {
            ...
            if (actionToRedo.measurementData) {
              ...
              strokeMeasurements[currentImageLabel][actionToRedo.label] =
                actionToRedo.measurementData;
      ...
      function resizeCanvas() {
        // If Fabric.js is managing the canvas, let it handle resizing
        // Direct canvas.width/height modification interferes with Fabric.js coordinate calculations
        if (window.app?.canvasManager?.fabricCanvas) {
          // Fabric.js is active - delegate to its resize method and skip direct manipulation
        ...
        console.log(
          `[resizeCanvas] Container: ${rect ? `${rect.width}x${rect.height}` : 'none'}, Target: ${targetWidth}x${targetHeight}, Window: ${window.innerWidth}x${window.innerHeight}`
        );
      ...
      function getStrokeAnchorPoint(strokeName, imageLabel = null) {
        ...
        console.log(
          `[ANCHOR] Stroke ${strokeName} bounds: (${minX.toFixed(1)},${minY.toFixed(1)}) to (${maxX.toFixed(1)},${maxY.toFixed(1)}) -> center: (${centerX.toFixed(1)},${centerY.toFixed(1)})`
        );
      ...
      function drawArrowLinePreview(startPoint, endPoint) {
        ...
        if (
          lineLength > 0 &&
          arrowSettings &&
          (arrowSettings.startArrow || arrowSettings.endArrow)
        ) {
          const unitX = dx / lineLength;
          const unitY = dy / lineLength;
      ...
      if (startArrowToggle || endArrowToggle) {
      ...
      // Note: Freehand mode removed - cycle is now: straight -> curved -> straight
      drawingModeToggle.addEventListener('click', () => {
        if (drawingMode === 'straight' || drawingMode === 'freehand') {
          // straight (or legacy freehand) -> curved
          drawingMode = 'curved';
      ...
      [startArrowToggle, endArrowToggle].filter(Boolean).forEach(toggle => {
      ...
      function calculateFitScale(fitMode) {
        ...
        if (captureEl && canvasRect) {
          ...
          if (cssWidth > 0 && cssHeight > 0) {
            ...
            console.log(
              `[calculateFitScale] Using capture frame: ${cssWidth}x${cssHeight}, offsets(canvas px)=(${offsetXCanvas.toFixed(1)}, ${offsetYCanvas.toFixed(1)})`
            );
          }
        }
      ...
      function findLabelAtPoint(x, y) {
        for (const label of currentLabelPositions) {
          ...
          if (x >= left && x <= left + label.width && y >= top && y <= top + label.height) {
            return label;
          }
        }
        return null;
      ...
      function bindCanvasListeners() {
        ...
        canvas.addEventListener('scalechange', onCanvasScaleChange, {
          signal: eventListeners.signal,
        });
      ...
      function onCanvasMouseDown(e) {
        ...
        console.log(
          '🔄 [DEBUG] Canvas mousedown - curveJustCompleted:',
          curveJustCompleted,
          'target:',
          e.target,
          'tagName:',
        ...
        console.log(
          '🔄 [DEBUG] Canvas label click - clickedCanvasLabel:',
          clickedCanvasLabel,
          'isClickOnCanvasLabel:',
          isClickOnCanvasLabel
        );
        console.log(
          '🔄 [DEBUG] About to check conditions - curveJustCompleted:',
          curveJustCompleted
        );
        ...
        console.log(
          `🔄 [DEBUG] Canvas click timing - now: ${now}, lastCanvasClickTime: ${window.lastCanvasClickTime}, timeSinceLastClick: ${timeSinceLastClick}, clickDelay: ${window.clickDelay}`
        );
        if (timeSinceLastClick < window.clickDelay) {
          ...
          console.log(
            `🔄 [DEBUG] Double-click check - clickedLabelForDoubleClick:`,
            clickedLabelForDoubleClick,
            `lastClickedCanvasLabel: ${window.lastClickedCanvasLabel}`
          );
          if (
            clickedLabelForDoubleClick &&
            ...
            console.log(
              `🔄 [DEBUG] Double-click detected on label ${clickedLabelForDoubleClick.strokeLabel} - entering edit mode`
            );
      ...
      function onCanvasMouseMove(e) {
          ...
          requestAnimationFrame(() => {
            ...
            if (draggingAnchor && dragCurveStroke && dragAnchorIndex >= 0) {
              ...
              if (
                vectorData &&
                vectorData.controlPoints &&
                vectorData.controlPoints[dragAnchorIndex]
              ) {
        ...
        if (isDraggingLabel) {
          ...
          if (vectorData && vectorData.points.length > 0) {
            ...
            let currentOffset =
              customLabelPositions[currentImageLabel][strokeName] ||
            ...
            if (!currentOffset) {
              ...
              const currentLabelRect = currentLabelPositions.find(
                l => l.strokeLabel === strokeName
              );
              if (currentLabelRect) {
            ...
            if (!window.customLabelAbsolutePositions[currentImageLabel])
              window.customLabelAbsolutePositions[currentImageLabel] = {};
        ...
        if (isDrawing) {
          ...
          if (drawingMode === 'straight' && straightLineStart) {
            ...
            if (dragDistance > MINIMUM_DRAG_DISTANCE) {
              ...
              const startTransformed = getTransformedCoords(
                straightLineStart.x,
                straightLineStart.y
              );
            ...
            if (dragDistance > MINIMUM_DRAG_DISTANCE && points.length > 0) {
              ...
              if (points.length > 0) {
                ...
                const endPointStrokeData = checkForStrokeAtPoint(
                  lastPoint.canvasX,
                  lastPoint.canvasY
                );
      ...
      document.addEventListener('keydown', e => {
        if (e.key === 'Tab') {
          ...
          if (
            ...
            // Cycle through modes: straight → curved → straight
            if (drawingMode === 'straight' || drawingMode === 'freehand') {
              // straight (or legacy freehand) -> curved
              drawingMode = 'curved';
              drawingModeToggle.classList.remove('straight-mode');
          ...
          if (captureFrame) {
            ...
            if (
              ...
              cropData = {
                ...
                width: Math.min(
                  canvas.width,
                  (frameRect.right - Math.max(frameRect.left, canvasRect.left)) * dpr
                ),
                height: Math.min(
                  canvas.height,
                  (frameRect.bottom - Math.max(frameRect.top, canvasRect.top)) * dpr
                ),
              };
            }
          }
      ...
      if (!saveButton) {
        console.warn('[PAINT.JS] Save button not found!');
      } else {
        saveButton.addEventListener('click', () => {
          ...
          // Try Fabric.js canvas first (new system)
          ...
          if (fabricCanvas) {
            try {
              ...
              if (captureEl && fabricCanvasEl) {
                ...
                if (cssWidth > 0 && cssHeight > 0) {
                  // Use Fabric's toDataURL with crop coordinates
                  const scale = fabricCanvasEl.width / canvasRect.width;
                  dataUrl = fabricCanvas.toDataURL({
                    format: 'png',
                    quality: 0.95,
                    left: Math.round((left - canvasRect.left) * scale),
                    top: Math.round((top - canvasRect.top) * scale),
              ...
              // Fallback to full Fabric canvas
              if (!dataUrl) {
                dataUrl = fabricCanvas.toDataURL({
                  format: 'png',
                  quality: 0.95,
              ...
              console.error('[Save] Error exporting Fabric canvas:', err);
            }
          }

          // Fallback to legacy canvas if Fabric export failed
          if (!dataUrl && canvas) {
            const captureEl = document.getElementById('captureFrame');
          ...
          if (!dataUrl) {
            alert('Unable to save image. No canvas available.');
            return;
          }
      ...
      const handleFiles = files => {
        ...
        sortedFiles.forEach((file, index) => {
          if (file.type.indexOf('image') !== -1) {
            ...
            // Use window.addImageToSidebar to allow hook interception for Fabric.js integration
            window.addImageToSidebar(url, label, displayName);
            ...
            const promise = pasteImageFromUrl(url, label).catch(err => {
              console.error(
                `[handleFiles] Error loading image ${label} via pasteImageFromUrl:`,
                err
              );
            });
      ...
      if (!pasteButton.__uploadBound) {
        pasteButton.__uploadBound = true;
      ...
      window.rotateImage = function (imageIndex, degrees) {
        ...
        console.log(
          `[rotateImage] Updated rotation for ${currentLabel}: ${((currentRotation * 180) / Math.PI).toFixed(1)}° + ${degrees}° = ${((window.imageRotationByLabel[currentLabel] * 180) / Math.PI).toFixed(1)}°`
        );
      ...
      window.transformImageData = function (imageLabel, operation, value, width, height) {
        ...
        if (operation === 'rotate') {
          ...
          if (window.vectorStrokesByImage && window.vectorStrokesByImage[imageLabel]) {
            ...
            console.log(
              `[transformImageData] Flipped ${Object.keys(strokes).length} strokes ${value}`
            );
      ...
      document.addEventListener('keydown', e => {
        ...
        if (e.key === 'q' || e.key === 'Q') {
          ...
          const currentScale = window.imageScaleByLabel[currentImageLabel];
          const scales = [
            0.1, 0.15, 0.2, 0.25, 0.33, 0.5, 0.67, 0.75, 0.8, 0.9, 1, 1.1, 1.25, 1.5, 1.75, 2, 2.5,
            3, 4, 5,
          ];
      ...
      window.addEventListener('resize', () => {
        ...
        if (imageSidebar && strokeSidebar) {
          ...
          if (
            imageSidebarRect.left < canvasRect.right &&
            imageSidebarRect.right > canvasRect.left
          ) {
          ...
          if (
            strokeSidebarRect.right > canvasRect.left &&
            strokeSidebarRect.left < canvasRect.right
          ) {
            // Added check for actual overlap
      ...
      document.addEventListener('paste', e => {
        ...
        for (let i = 0; i < items.length; i++) {
          ...
          if (item.type.indexOf('image') !== -1) {
            ...
            window.labelCounters[baseLabelForPasted] =
              (window.labelCounters[baseLabelForPasted] || 0) + 1;
            ...
            // Use window.addImageToSidebar to allow hook interception for Fabric.js integration
            window.addImageToSidebar(url, newImageLabel, displayName);
            ...
            pasteImageFromUrl(url, newImageLabel)
              ...
              .catch(err => {
                console.error(
                  `[Paste Handler] Error in pasteImageFromUrl for ${newImageLabel}:`,
                  err
                );
              });
      ...
      function deleteSelectedStrokes() {
        ...
        deletedStrokeLabels.forEach(strokeLabel => {
          ...
          if (strokeVisibilityByImage[currentImageLabel]) {
            originalVisibility[strokeLabel] =
              strokeVisibilityByImage[currentImageLabel][strokeLabel];
          }
          if (strokeLabelVisibility[currentImageLabel]) {
      ...
      function onCanvasDoubleClick(e) {
        ...
        if (drawingMode === 'curved' && curvedLinePoints.length >= 2) {
          ...
          if (strokeAtPoint) {
            ...
            const nearestPoint = findNearestPointOnStroke(
              e.offsetX,
              e.offsetY,
              strokeAtPoint.label
            );
    ...
    })(); // End of IIFE (was nested DOMContentLoaded)
    ...
    window.syncCustomLabelPositions = function (imageLabel) {
      ...
      console.log(
        `[SYNC] Custom positions synchronization for ${img} - window storage initialized`
      );
    };
    ...
    window.shareProject = async function () {
      ...
      try {
        ...
        const labelsFromState =
          ...
          Array.isArray(window.paintApp.state.imageLabels)
          ...
          if (window.customLabelOffsetsRotationByImageAndStroke) {
            ...
            imageLabels.forEach(label => {
              projectData.customLabelRotationStamps[label] = window
                ...
                ? JSON.parse(
                    JSON.stringify(window.customLabelOffsetsRotationByImageAndStroke[label])
                  )
                : {};
            });
          }
    ...
    window.updateSharedProject = async function () {
      try {
        ...
        if (typeof window.projectManager?.showStatusMessage === 'function') {
          window.projectManager.showStatusMessage(
            'Failed to update share: ' + error.message,
            'error'
          );
        } else {
