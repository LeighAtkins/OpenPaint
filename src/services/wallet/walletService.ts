...
class WalletService {
  ...
  async loadWallet(): Promise<WalletState> {
    ...
    const auth = this.getStoredAuth();
    if (this.authBlocked) {
      if (!auth || auth.accessToken === this.blockedAuthToken) {
        return this.state;
      }
      this.authBlocked = false;
      this.blockedAuthToken = '';
    }
    ...
    this.loadPromise = (async () => {
      try {
        ...
        if (walletError?.code === 'not_authenticated' || walletError?.status === 401) {
          const latestAuth = this.getStoredAuth();
          this.authBlocked = true;
          this.blockedAuthToken = latestAuth?.accessToken || '';
          if (this.loadRetryTimeout) {
            clearTimeout(this.loadRetryTimeout);
            this.loadRetryTimeout = null;
          }
  ...
  isLoaded(): boolean {
    return this.loaded || this.authBlocked;
  }
