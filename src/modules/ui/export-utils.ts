...
window.showPDFExportDialog = async function (projectName) {
  ...
  overlay.innerHTML = `
    <div style="background:white;border-radius:12px;padding:30px;max-width:500px;box-shadow:0 10px 40px rgba(0,0,0,0.3);">
      ...
      <p style="color:#666;margin-bottom:15px;">Creating PDF with ${viewIds.length} page(s) and editable form fields.</p>
      <div style="margin-bottom:15px;">
        <label style="display:block;margin-bottom:5px;font-weight:600;color:#333;">Image Quality:</label>
      ...
      <div style="margin-bottom:15px;">
        <label style="display:block;margin-bottom:5px;font-weight:600;color:#333;">Page Size:</label>
      ...
      <label style="display:flex;align-items:center;gap:8px;cursor:pointer;margin-bottom:20px;">
        ...
        <span style="color:#333;">Include editable measurement fields</span>
      </label>
      <div style="display:flex;gap:10px;">
        <button id="generatePdfBtn" style="flex:1;padding:12px;background:#3b82f6;color:white;border:none;border-radius:8px;font-weight:600;cursor:pointer;">Generate PDF</button>
        <button id="cancelPdfBtn" style="flex:1;padding:12px;background:#6b7280;color:white;border:none;border-radius:8px;font-weight:600;cursor:pointer;">Cancel</button>
      </div>
...
async function generatePDFWithPDFLib(projectName, viewIds, quality, pageSize, includeMeasurements) {
    ...
    if (includeMeasurements) {
      ...
      if (strokes.length > 0) {
        ...
        strokes.forEach((strokeLabel, idx) => {
          ...
          // Label
          page.drawText(strokeLabel, { x, y: yPos, size: 8, font: fontBold, color: rgb(0, 0, 0) });
