...
export function initPetsSystem(): void {
  ...
  // Keep wallet state in sync on login and restore the currently equipped pet.
  // Cleanup happens first so reload/HMR cannot leave stale sprites behind.
  authService.onAuthStateChange(user => {
    authMountRunId += 1;
    const runId = authMountRunId;

    if (user) {
      void (async () => {
        const state = await walletService.loadWallet();
        if (runId !== authMountRunId) return;

        petFollowerRenderer.cleanupDomArtifacts();
        if (state.equippedPet) {
          await petFollowerRenderer.mount(state.equippedPet);
        } else {
          petFollowerRenderer.unmount();
        }
      })();
