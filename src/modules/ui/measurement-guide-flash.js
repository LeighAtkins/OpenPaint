...
function getGuideSplitWorkspaceMode() {
  try {
    return window.getMeasurementSplitWorkspaceState?.()?.activeMode === 'measurement-edit'
      ? 'measurement-edit'
      : 'guide-compare';
  } catch {
    return 'guide-compare';
  }
}
...
function syncGuideSplitWorkspaceModeUi() {
  ...
  const measurementModeActive = guideSplitEnabled && isMeasurementSplitMode();
  if (wrapper) {
    if (guideSplitEnabled) {
      wrapper.dataset.guideSplitMode = measurementModeActive ? 'measurement-edit' : 'guide-compare';
    } else {
      delete wrapper.dataset.guideSplitMode;
    }
  }
  document.body.classList.toggle('measurement-split-workspace-active', measurementModeActive);
}
...
function cleanupGuideSplitPanelVisibility() {
  const strokePanel = document.getElementById('strokePanel');
  const imagePanel = document.getElementById('imagePanel');
  const elementsBody = document.getElementById('elementsBody');
  const imagePanelContent = document.getElementById('imagePanelContent');

  [strokePanel, imagePanel, elementsBody, imagePanelContent].forEach(element => {
    if (!element) return;
  ...
  strokePanel?.style.removeProperty('display');
  imagePanel?.style.removeProperty('display');

  if (elementsBody && !strokePanel?.classList.contains('collapsed')) {
    elementsBody.classList.remove('hidden');
    elementsBody.style.removeProperty('display');
  }

  if (imagePanelContent && !imagePanel?.classList.contains('collapsed')) {
    imagePanelContent.classList.remove('hidden');
    imagePanelContent.style.removeProperty('display');
  }
}
...
function getGuideSplitActiveFrameWorldRect(viewId) {
  const normalizedViewId = toBaseViewId(viewId || getCurrentViewId());
  const tabState = window.captureTabsByLabel?.[normalizedViewId];
  const activeTabId = tabState?.activeTabId || tabState?.lastNonMasterId || null;
  const activeTab = Array.isArray(tabState?.tabs)
    ? tabState.tabs.find(tab => tab?.id === activeTabId) || null
    : null;
  const worldRect = activeTab?.type !== 'master' ? activeTab?.captureFrame?.worldRect : null;
  return isValidGuideSplitWorldRect(worldRect) ? worldRect : null;
}
...
function captureGuideSplitRestoreSnapshot(viewId) {
  ...
  const savedImageWorldRect = isValidGuideSplitWorldRect(
    manager?.views?.[normalizedViewId]?.backgroundWorldRect
  )
    ? manager.views[normalizedViewId].backgroundWorldRect
    : null;
  guideSplitRestoreSnapshot = {
    ...
    imageWorldRect: cloneGuideSplitValue(savedImageWorldRect || getGuideSplitImageWorldRect()),
  };
  window.__guideSplitRestoreSnapshot = cloneGuideSplitValue(guideSplitRestoreSnapshot);
}

function isValidGuideSplitWorldRect(rectLike) {
...
function restoreGuideSplitSnapshot(viewId) {
  ...
  const manager = window.app?.projectManager || window.projectManager;
  const view = manager?.views?.[normalizedViewId];
  ...
  // The normal-view reload path reads from projectManager.views, not only the
  // live capture-tab cache. Keep the saved view in step with the live restore
  // so setBackgroundImage can reuse the authoritative pre-split placement.
  if (view && guideSplitRestoreSnapshot.tabs) {
    view.tabs = cloneGuideSplitValue(guideSplitRestoreSnapshot.tabs);
  }
  if (view && guideSplitRestoreSnapshot.viewport) {
    view.viewport = cloneGuideSplitValue(guideSplitRestoreSnapshot.viewport);
  }
  const restoreWorldRect = isValidGuideSplitWorldRect(guideSplitRestoreSnapshot.imageWorldRect)
    ? guideSplitRestoreSnapshot.imageWorldRect
    : getGuideSplitImageWorldRect();
  if (view && isValidGuideSplitWorldRect(restoreWorldRect)) {
    view.backgroundWorldRect = cloneGuideSplitValue(restoreWorldRect);
  }
...
function fitGuideSplitPrimaryBackgroundToFrame() {
  const primaryCanvasManager = getPrimaryCanvasManager();
  if (window.__guideSplitUserViewportDirty === true) {
    return false;
  }
...
function ensureStyles() {
  ...
  style.textContent = `
    ...
    .guide-split-root {
      ...
      width: 100%;
      height: calc(100% - var(--toolbar-height, 48px) - 8px);
      min-height: 0;
      margin-top: calc(var(--toolbar-height, 48px) + 8px);
    }
    .guide-split-pane {
      position: relative;
      min-width: 0;
    ...
    #main-canvas-wrapper.guide-split-active[data-guide-split-mode="measurement-edit"] .guide-split-root {
    ...
    #main-canvas-wrapper.guide-split-active[data-guide-split-mode="measurement-edit"] .guide-split-guide-pane {
      border-left-color: rgba(148, 163, 184, 0.35);
      background:
        linear-gradient(180deg, rgba(255, 255, 255, 0.96) 0%, rgba(243, 247, 255, 0.98) 100%);
    }
    ...
    .guide-split-measurement-editor-host {
      display: flex;
      flex-direction: column;
      min-width: 0;
      min-height: 0;
    ...
    body.measurement-split-workspace-active #strokePanel {
      display: none !important;
      visibility: hidden !important;
      opacity: 0 !important;
      pointer-events: none !important;
    }
    body.measurement-split-workspace-active #elementsHeader {
      position: sticky;
      top: 0;
      z-index: 5;
      padding: 18px 20px;
    ...
    body.measurement-split-workspace-active #elementsBody {
      display: flex !important;
      flex-direction: column;
      min-height: 0 !important;
      background: transparent !important;
    }
    body.measurement-split-workspace-active #elementsControls {
      position: sticky;
      top: 0;
      z-index: 4;
    ...
    body.measurement-split-workspace-active #strokeVisibilityControls {
      padding: 10px 16px 32px !important;
      overflow: auto;
    }
    body.measurement-split-workspace-active #strokeVisibilityControls #strokesList {
      padding: 0 0 32px !important;
    }
    body.measurement-split-workspace-active #strokeVisibilityControls h4 {
    ...
    body.measurement-split-workspace-active .stroke-visibility-item {
    ...
    body.measurement-split-workspace-active .stroke-label-container {
      gap: 10px;
      align-items: center;
    }
    body.measurement-split-workspace-active .stroke-name {
      padding: 6px 10px;
      border-radius: 10px;
      font-size: 14px;
      font-weight: 800;
    }
    body.measurement-split-workspace-active .stroke-measurement {
      min-width: 92px;
      padding: 8px 10px;
      border-radius: 10px;
      border: 1px solid rgba(148, 163, 184, 0.35);
    ...
    body.measurement-split-workspace-active .stroke-label-toggle-btn {
      min-width: 34px;
      min-height: 34px;
      border-radius: 10px;
    }
    body.measurement-split-workspace-active .delete-image-btn {
...
function setGuideSplitEnabled(enabled) {
  ...
  if (next) {
    window.__guideSplitUserViewportDirty = false;
    window.__guideSplitUserViewportChangedAt = 0;
    ...
    const currentViewId = toBaseViewId(pm?.currentViewId || getCurrentViewId());
    const stableBackgroundWorldRect = isValidGuideSplitWorldRect(
      pm?.views?.[currentViewId]?.backgroundWorldRect
    )
      ? cloneGuideSplitValue(pm.views[currentViewId].backgroundWorldRect)
      : null;
    ...
    if (pm && cm) {
      ...
      if (pm.views?.[pm.currentViewId]) {
        const rectToKeep = stableBackgroundWorldRect || currentRect;
        if (isValidGuideSplitWorldRect(rectToKeep)) {
          pm.views[pm.currentViewId].backgroundWorldRect = JSON.parse(JSON.stringify(rectToKeep));
        }
      }
    ...
    if (typeof window.captureTabsSyncActive === 'function') {
      window.captureTabsSyncActive(guideCompareWorkspaceState.leftViewId || getCurrentViewId(), {
        preserveWorldRect: true,
      });
    } else if (typeof window.saveCurrentCaptureFrameForLabel === 'function') {
...
async function applyGuideSplitLayout() {
  ...
  if (guideSplitEnabled) {
    ...
    window.restoreMeasurementSplitStrokePanel?.();
    ...
    syncGuideSplitWorkspaceModeUi();
    document.body.classList.remove('measurement-split-workspace-active');
    restoreGuideSplitPanelSnapshot();
    cleanupGuideSplitPanelVisibility();
...
function renderGuideSplitPane() {
  ...
  if (workspaceMode === 'measurement-edit') {
    const workspaceViewId =
      measurementWorkspaceState?.activeImportedViewId || leftViewId || getCurrentViewId() || '';
    const workspaceImage = imageRows.find(row => row.id === workspaceViewId);
    const workspaceLabel = workspaceImage?.displayName || workspaceViewId || 'Current View';

    pane.innerHTML = `
      <header class="guide-split-guide-head">
        <span>CW Measurement Mapping</span>
    ...
    `;
    window.renderCwMeasurementWorkspacePane?.();
    window.dispatchEvent(
      new CustomEvent('openpaint:guide-split-pane-rendered', {
        detail: {
          mode: 'measurement-edit',
        },
      })
    );
  } else if (!rightViewId) {
...
export function initMeasurementGuideFlash() {
  ...
  window.addEventListener('openpaint:measurement-split-workspace-change', () => {
    ...
    window.__guideSplitUserViewportDirty = false;
    window.__guideSplitUserViewportChangedAt = 0;
    getPrimaryCanvasManager()?.enforceFloatingLayout?.();
    renderGuideSplitPane();
    scheduleGuideSplitLayoutSync({ settle: true });
  });
