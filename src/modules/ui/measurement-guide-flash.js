...
function ensureStyles() {
  ...
  style.textContent = `
    ...
    .guide-gallery-viewchip:hover {
      background: #eff6ff;
      border-color: #93c5fd;
      color: #1d4ed8;
    }
    .guide-gallery-viewchip.active {
      background: #dbeafe;
      border-color: #3b82f6;
      color: #1e3a8a;
    }
    ...
    guideCodeListPromise = fetch('/api/measurement-guides/codes', { method: 'GET' })
      ...
      .catch(() => {
        ...
        fallback.forEach(code => {
          fallbackViewsByCode[code] = ['front'];
        });
...
function getGuideModelLinkState() {
  ...
  return { selections, linksByImage };
}
...
function saveGuideModelLinkState(state) {
  ...
  const payload = {
    ...
    measurementGuideModelLinksByImage: state.linksByImage,
  };
...
function linkSelectionToImage(selectionId, imageId) {
  ...
  if (!normalizedSelectionId || !normalizedImageId) return;

  const linksByImage = { ...state.linksByImage };
  Object.keys(linksByImage).forEach(key => {
    if (linksByImage[key] === normalizedSelectionId || key === normalizedImageId) {
      delete linksByImage[key];
    }
  });
  linksByImage[normalizedImageId] = normalizedSelectionId;
  state.linksByImage = linksByImage;
  saveGuideModelLinkState(state);
}
...
function removeModelSelection(selectionId) {
  ...
  const linksByImage = { ...state.linksByImage };
  Object.keys(linksByImage).forEach(imageId => {
    if (linksByImage[imageId] === normalizedSelectionId) {
      delete linksByImage[imageId];
    }
  });
  state.linksByImage = linksByImage;
  saveGuideModelLinkState(state);
...
function unlinkImageModel(imageId) {
  ...
  const state = getGuideModelLinkState();
  if (!state.linksByImage[normalizedImageId]) return;
  const linksByImage = { ...state.linksByImage };
  delete linksByImage[normalizedImageId];
  state.linksByImage = linksByImage;
  saveGuideModelLinkState(state);
}
...
function showGuideGallery() {
  ...
  let galleryMode = 'select';
  let selectedModelIds = new Set();
  let bindPreviewImageId = '';
  let bindPreviewSelectionId = '';
  ...
  const bindCodeToTarget = (
    ...
    linkSelectionToImage(selection.id, targetViewId);
    saveGuideCodes([code], targetViewId);
    tagGuideOnView(targetViewId, code, variant);
    setStatusMessage(`Bound ${code} to image ${targetViewId}.`, 'success');
    window.dispatchEvent(new Event('openpaint:guide-binding-changed'));
  };
  ...
  const renderGallery = () => {
    ...
    if (!bindPreviewImageId || !projectImages.some(image => image.id === bindPreviewImageId)) {
      bindPreviewImageId = projectImages[0]?.id || '';
    }
    const linkedSelectionForPreviewImage = modelState.linksByImage[bindPreviewImageId] || '';
    if (!bindPreviewSelectionId || !selectionById.has(bindPreviewSelectionId)) {
      bindPreviewSelectionId = linkedSelectionForPreviewImage || modelState.selections[0]?.id || '';
    }
    const bindPreviewSelection = bindPreviewSelectionId
      ? selectionById.get(bindPreviewSelectionId)
      : null;
        ...
        return `
          <div class="guide-gallery-item${selectedClass}" data-code="${code}">
            <div class="guide-gallery-item-label">
              <span>
                <input type="checkbox" class="guide-gallery-item-select" data-select-code="${code}" data-select-view="${activeView}" ${modelState.selections.some(item => item.code === code && item.variant === activeView) ? 'checked' : ''} />
                ${code}
              </span>
              <div class="guide-gallery-viewchips">
                ${views
                  .map(view => {
                    const active = code === selectedCode && view === activeView ? ' active' : '';
                    return `<button type="button" class="guide-gallery-viewchip${active}" data-view-chip="${view}" data-code="${code}">${view.toUpperCase()}</button>`;
                  })
                  .join('')}
              </div>
            </div>
    ...
    const selectedModelRows = modelState.selections
      .map(selection => {
        const linkedImageId =
          Object.entries(modelState.linksByImage).find(([, id]) => id === selection.id)?.[0] || '';
        const linkedImageLabel = linkedImageId
          ? imageDisplayById[linkedImageId] || linkedImageId
          : '';
        const isChecked = selectedModelIds.has(selection.id) ? 'checked' : '';
        return `
          <li class="guide-gallery-link-row">
            <label style="display:flex;align-items:center;gap:6px;min-width:0;">
              <input type="checkbox" data-select-model-id="${selection.id}" ${isChecked} />
              <span style="min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"><strong>${selection.code}</strong> · ${selection.variant.toUpperCase()}</span>
            </label>
            <span class="guide-gallery-link-pill ${linkedImageId ? '' : 'model-only'}">${linkedImageId ? `Linked -> ${linkedImageLabel}` : 'Model only'}</span>
          </li>
        `;
      })
      .join('');
    ...
    const imageRows = projectImages
      .map(image => {
        const linkedSelectionId = modelState.linksByImage[image.id] || '';
        const linkedSelection = linkedSelectionId ? selectionById.get(linkedSelectionId) : null;
        return `<tr><td><strong>${image.displayName}</strong></td><td><span class="guide-gallery-link-pill ${linkedSelection ? '' : 'model-only'}">${linkedSelection ? `${linkedSelection.code} · ${linkedSelection.variant.toUpperCase()}` : 'Unlinked image'}</span><span class="guide-gallery-link-actions"><button type="button" class="guide-gallery-link-btn" data-link-action="bind" data-image-id="${image.id}">Bind Selected</button><button type="button" class="guide-gallery-link-btn unlink" data-link-action="unlink" data-image-id="${image.id}">Unlink</button><button type="button" class="guide-gallery-link-btn" data-link-action="focus" data-image-id="${image.id}">Open Image</button></span></td></tr>`;
      })
      .join('');
      ...
      <section class="guide-gallery-bind-preview">
        <div class="guide-gallery-bind-preview-main">
          ...
          <div class="guide-gallery-bind-preview-actions">
            <button type="button" class="guide-gallery-link-btn" data-link-action="bind-preview" ${bindPreviewImage && bindPreviewSelection ? '' : 'disabled'}>Bind Now</button>
            ...
            <button type="button" class="guide-gallery-link-btn" data-link-action="focus-preview" ${bindPreviewImage ? '' : 'disabled'}>Open Image</button>
          </div>
          <div>
        ...
        <div class="guide-gallery-bind-preview-strip">
          ...
          <div>
            ...
            <div class="guide-gallery-bind-strip">${
              modelState.selections
                .map(
                  selection =>
                    `<button type="button" class="guide-gallery-bind-strip-item ${selection.id === bindPreviewSelectionId ? 'active' : ''}" data-bind-preview-selection="${selection.id}"><img src="${buildGuideUrl(selection.code, selection.variant)}" alt="${selection.code} ${selection.variant}" /><span>${selection.code} · ${selection.variant.toUpperCase()}</span></button>`
                )
                .join('') ||
    ...
    const renderSelectionLists = () => {
      ...
      const selectedRows = state.selections
        .map(selection => {
          const linkedImageId =
            Object.entries(state.linksByImage).find(([, id]) => id === selection.id)?.[0] || '';
          const linkedImageLabel = linkedImageId ? displayById[linkedImageId] || linkedImageId : '';
          const isChecked = selectedModelIds.has(selection.id) ? 'checked' : '';
          return `<li class="guide-gallery-link-row"><label style="display:flex;align-items:center;gap:6px;min-width:0;"><input type="checkbox" data-select-model-id="${selection.id}" ${isChecked} /><span style="min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"><strong>${selection.code}</strong> · ${selection.variant.toUpperCase()}</span></label><span class="guide-gallery-link-pill ${linkedImageId ? '' : 'model-only'}">${linkedImageId ? `Linked -> ${linkedImageLabel}` : 'Model only'}</span></li>`;
        })
        .join('');
      ...
      const tableRows = images
        .map(image => {
          const linkedSelectionId = state.linksByImage[image.id] || '';
      ...
      galleryOverlay.querySelectorAll('[data-select-model-id]').forEach(input => {
        input.onchange = () => {
          const selectionId = String(input.getAttribute('data-select-model-id') || '').trim();
          if (!selectionId) return;
          if (input.checked === true) {
            if (galleryMode === 'bind') {
              selectedModelIds = new Set([selectionId]);
            } else {
              selectedModelIds.add(selectionId);
            }
          } else {
            selectedModelIds.delete(selectionId);
          }
    ...
    const refreshBindPreviewDom = () => {
      ...
      const linkedSelectionId = bindPreviewImageId
        ...
        : '';
      if (!bindPreviewSelectionId || !byId.has(bindPreviewSelectionId)) {
        bindPreviewSelectionId = linkedSelectionId || state.selections[0]?.id || '';
      }

      const image = images.find(item => item.id === bindPreviewImageId) || null;
      const selection = bindPreviewSelectionId ? byId.get(bindPreviewSelectionId) : null;
      ...
      galleryOverlay.querySelectorAll('[data-bind-preview-selection]').forEach(button => {
        const selectionId = String(button.getAttribute('data-bind-preview-selection') || '').trim();
        button.classList.toggle('active', selectionId === bindPreviewSelectionId);
      });
      ...
      const focusBtnPreview = galleryOverlay.querySelector('[data-link-action="focus-preview"]');
      if (bindBtnPreview) bindBtnPreview.disabled = !(image && selection);
      if (unlinkBtnPreview) unlinkBtnPreview.disabled = !image;
      if (focusBtnPreview) focusBtnPreview.disabled = !image;
    };

    const hasSelection = Boolean(selectedCode);
    const hasSingleModelSelection = selectedModelIds.size === 1;
    [loadCurrentBtn, addImageBtn, addAllBtn].forEach(btn => {
      if (!btn) return;
      btn.disabled = !hasSelection;
    });
    if (bindBtn) {
      bindBtn.disabled = !(hasSelection || hasSingleModelSelection);
    }
    if (addSelectedBtn) {
      addSelectedBtn.disabled = selectedModelIds.size === 0;
      addSelectedBtn.textContent =
    ...
    const resolveBindingCandidate = () => {
      const state = getGuideModelLinkState();
      const byId = new Map(state.selections.map(item => [item.id, item]));
      if (galleryMode === 'bind' && bindPreviewSelectionId) {
        const previewModel = byId.get(bindPreviewSelectionId);
        if (previewModel) {
          return {
            selectionId: previewModel.id,
            code: previewModel.code,
            variant: previewModel.variant,
          };
        }
      }
    ...
    bindBtn?.addEventListener('click', event => {
      ...
      bindCodeToTarget(candidate.code, bindTargetViewId, candidate.variant, candidate.selectionId);
      renderSelectionLists();
    });
    ...
    galleryOverlay.onclick = async event => {
      ...
      if (actionBtn) {
        ...
        if (action === 'bind') {
          ...
          bindCodeToTarget(candidate.code, imageId, candidate.variant, candidate.selectionId);
          renderSelectionLists();
          return;
        }
        ...
        if (action === 'focus') {
          ...
          try {
            if (window.app?.projectManager?.switchView) {
              await window.app.projectManager.switchView(imageId, true);
              setStatusMessage(`Opened ${imageId}.`, 'info');
            }
          } catch (error) {
            console.warn('[Guide] Failed to switch to image', imageId, error);
            setStatusMessage(`Failed to open ${imageId}.`, 'warning');
          }
          return;
        }
        ...
        if (action === 'unlink-preview') {
          ...
          unlinkImageModel(bindPreviewImageId);
        ...
        if (action === 'focus-preview') {
          ...
          try {
            if (window.app?.projectManager?.switchView) {
              await window.app.projectManager.switchView(bindPreviewImageId, true);
              setStatusMessage(`Opened ${bindPreviewImageId}.`, 'info');
            }
          } catch (error) {
            console.warn('[Guide] Failed to switch to image', bindPreviewImageId, error);
            setStatusMessage(`Failed to open ${bindPreviewImageId}.`, 'warning');
          }
          return;
        }
      }
      ...
      if (previewImageBtn) {
        ...
        const state = getGuideModelLinkState();
        const linkedSelectionId = state.linksByImage[imageId] || '';
        if (linkedSelectionId) {
          bindPreviewSelectionId = linkedSelectionId;
          selectedModelIds = new Set([linkedSelectionId]);
        }
        refreshBindPreviewDom();
        return;
      }
      ...
      if (previewSelectionBtn) {
        ...
        if (!selectionId) return;
        bindPreviewSelectionId = selectionId;
        selectedModelIds = new Set([selectionId]);
        refreshBindPreviewDom();
      }
    };
    ...
    galleryOverlay.querySelectorAll('.guide-gallery-item').forEach(item => {
      item.addEventListener('click', event => {
        ...
        if (event.target?.closest('.guide-gallery-viewchip')) return;
    ...
    galleryOverlay.querySelectorAll('[data-select-code]').forEach(input => {
      ...
      input.addEventListener('change', () => {
        ...
        const variant = input.getAttribute('data-select-view') || 'front';
        if (!code) return;
        ...
        if (input.checked === true) {
          ...
          selectedModelIds.delete(selection.id);
          removeModelSelection(selection.id);
        }
        renderSelectionLists();
      });
    });

    galleryOverlay.querySelectorAll('.guide-gallery-viewchip').forEach(chip => {
      chip.addEventListener('mouseenter', () => {
        const code = chip.getAttribute('data-code') || '';
        const view = chip.getAttribute('data-view-chip') || 'front';
        const card = chip.closest('.guide-gallery-item');
        const img = card?.querySelector('img[data-guide-code]');
        if (img) {
          img.src = buildGuideUrl(code, view);
          img.dataset.guideView = view;
          img.alt = `${code} ${view} view`;
        }
      });
      chip.addEventListener('click', event => {
        ...
        selectedCode = code;
        selectedView = view;
        upsertModelSelection(code, view);
        renderGallery();
      });
    });
