...
/* global JSZip, generateMeasurementsList, saveAllImages */
  ...
  function saveProject() {
    try {
      ...
      if (window.orderedImageLabels && window.orderedImageLabels.length > 0) {
        // Deduplicate orderedImageLabels before saving to prevent future loading issues
        const seen = new Set();
        const deduplicatedOrder = window.orderedImageLabels.filter(label => {
          if (seen.has(label)) {
            console.warn(`[Save Project] Duplicate label in orderedImageLabels detected: ${label}, skipping duplicate`);
            return false;
          }
          seen.add(label);
          return true;
        });
        projectData.imageOrder = deduplicatedOrder;
            ...
            return projectJsonFile.async('string')
              .then(jsonContent => {
                ...
                if (imageOrder.length > 0) {
                  ...
                  const filteredOrder = imageOrder.filter(label => parsedProjectData.imageLabels.includes(label));
                  
                  // Deduplicate the filtered order while preserving sequence (keep first occurrence)
                  const seen = new Set();
                  labelsToProcess = filteredOrder.filter(label => {
                    if (seen.has(label)) {
                      console.warn(`[Project Load] Duplicate label in imageOrder detected: ${label}, skipping duplicate`);
                      return false;
                    }
                    seen.add(label);
                    return true;
