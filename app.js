...
// In Vercel serverless, use /tmp (writable), otherwise use project directory
const uploadDir = process.env.VERCEL 
    ? path.join('/tmp', 'uploads')
    : path.join(__dirname, 'uploads');

// Only create directory if it doesn't exist and parent directory exists
if (!fs.existsSync(uploadDir)) {
    try {
        // Check if parent directory exists (for Vercel /tmp should exist)
        const parentDir = path.dirname(uploadDir);
        if (fs.existsSync(parentDir) || process.env.VERCEL) {
            fs.mkdirSync(uploadDir, { recursive: true });
            console.log('Created uploads directory:', uploadDir);
        } else {
            console.warn('Cannot create uploads directory, parent does not exist:', parentDir);
        }
    } catch (error) {
        console.warn('Failed to create uploads directory:', error.message);
        // Continue anyway - multer will handle errors if directory doesn't exist
    }
}
