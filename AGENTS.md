...
- `bun run dev` is the preferred command to start the Vite dev server (frontend hot reload).
...
- **Never suggest browser refresh**: The user is a developer and knows to refresh. If a fix isn't working, suggesting refresh is patronizing and unhelpful. Focus on debugging the actual code issue instead.
- **Provide search terms for console output**: When asking the user to check the browser console, always provide a specific search term (e.g., "search for `[DEBUG]`" or "look for `zoom:`") so they can quickly find the relevant logs among all the output.
- Keep responses concise and technical.
- Avoid unnecessary explanations of what was done - the user can read the code.
