...
<body class="bg-gray-50 h-screen overflow-hidden">
    ...
    <div id="frame-capture" class="w-full" style="min-height: 100vh; overflow-y: visible; position: relative;">
    ...
    <div id="strokePanel" class="floating-panel fixed left-4 z-30 rounded-2xl w-64 transition-all duration-300 overflow-hidden" style="top: clamp(1rem, 50vh - 20rem, calc(100vh - 40rem - 1rem)); max-height: min(40rem, calc(100vh - 2rem));">
        ...
        <div id="elementsControls" class="px-3 pt-3 pb-1 border-b border-gray-100">
            <div class="flex gap-2 mb-2">
                ...
                <button id="importMeasurementsBtn" title="Import measurements from customer data" class="flex-1 px-2 py-1 text-xs bg-orange-100 hover:bg-orange-200 text-orange-700 rounded-lg transition-all font-medium">
                    Import List
                </button>
    ...
    <div id="strokePanelIcon" class="panel-toggle-icon" data-panel="strokePanel" style="display: none; opacity: 0;">✏️</div>
    <div id="imagePanelIcon" class="panel-toggle-icon" data-panel="imagePanel" style="display: none; opacity: 0;">🖼️</div>
    ...
    <script>
        ...
        function initializeTopToolbar() {
            ...
            if (labelBackgroundToggleBtn) {
                const backgroundStates = ['solid', 'clear-black', 'clear-selected', 'clear-white'];
                const stateLabels = ['Solid', 'Clear Black', 'Clear Color', 'Clear White'];
                
                const syncBackgroundBtn = () => {
                    const background = (window.paintApp?.state?.labelBackground) || 'solid';
                    const stateIndex = backgroundStates.indexOf(background);
                    const currentIndex = stateIndex >= 0 ? stateIndex : 0;
                    labelBackgroundToggleBtn.textContent = stateLabels[currentIndex];
                    labelBackgroundToggleBtn.setAttribute('aria-pressed', background !== 'solid' ? 'true' : 'false');
                };
                
                labelBackgroundToggleBtn.addEventListener('click', () => {
                    const background = (window.paintApp?.state?.labelBackground) || 'solid';
                    const currentIndex = backgroundStates.indexOf(background);
                    const nextIndex = (currentIndex + 1) % backgroundStates.length;
                    applyBackground(backgroundStates[nextIndex]);
                    syncBackgroundBtn();
                });
                syncBackgroundBtn();
            }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Import Measurements functionality
            const importMeasurementsBtn = document.getElementById('importMeasurementsBtn');
            if (importMeasurementsBtn) {
                importMeasurementsBtn.addEventListener('click', () => {
                    if (typeof window.showImportMeasurementsDialog === 'function') {
                        window.showImportMeasurementsDialog();
                    }
                });
            }
            ...
            const startSyncInterval = () => {
                syncInterval = setInterval(() => {
                    ...
                    if (currentLegacyCount > lastLegacyCount) {
                        ...
                        // After adding new images, only load the first one if no image is currently active
                        // This prevents overriding when user pastes a second image
                        if (imageGalleryData.length > 0 && !window.currentImageLabel) {
    ...
    <!-- Load AI Draw Bot after paint.js -->
    <script src="js/ai-draw-bot.js?v=20251116170000"></script>
    ...
    <script>
        (function initMiniStepper() {
            ...
            function updateActivePill() {
                const currentLabel = window.currentImageLabel || window.paintApp?.state?.currentImageLabel;
                if (!currentLabel) return;
                
                const stepButtons = Array.from(document.querySelectorAll('#mini-stepper button[data-target]'));
                ...
                // Auto-scroll the active pill to center using the same approach as thumbnail clicks
                if (activeButton) {
            ...
            function updatePills() {
                ...
                // Fallback: if no containers found but we have images in state, use state directly
                // This ensures pills load even when the image panel is closed
                let containersToUse = imageContainers;
                if (imageContainers.length === 0 && window.originalImages && Object.keys(window.originalImages).length > 0) {
                    // Create virtual containers from state
                    const imageLabels = Object.keys(window.originalImages).filter(label => 
                        !label.includes('Demo') && !label.includes('Blank')
                    );
                    
                    if (imageLabels.length > 0) {
                        // Try to find containers even if hidden/closed
                        containersToUse = imageLabels.map(label => {
                            const container = Array.from(document.querySelectorAll('.image-container, .image-thumbnail'))
                                .find(c => {
                                    const cLabel = c.dataset?.label || c.getAttribute('title') || c.dataset?.imageIndex || c.id;
                                    return cLabel === label;
                                });
                            return container;
                        }).filter(Boolean);
                        
                        // If still no containers (panel closed), create virtual containers from labels
                        if (containersToUse.length === 0) {
                            containersToUse = imageLabels.map(label => ({ 
                                dataset: { label: label },
                                getAttribute: (attr) => attr === 'title' ? label : null,
                                id: label,
                                tagName: 'DIV'
                            }));
                        }
                ...
                // Build pills for each image
                const pillsHTML = containersToUse.map((container, idx) => {
                    const label = container.dataset?.label || container.getAttribute('title') || container.dataset?.imageIndex || container.id || `Image ${idx + 1}`;
                    const n = idx + 1;
                    return `
    ...
    <!-- Load AI Draw Bot Integration after AI Draw Bot and paint.js are ready -->
    <script src="js/ai-draw-bot-integration.js?v=20251116170000"></script>

</body>
</html>
<script async data-explicit-opt-in="true" data-deployment-id="dpl_FsNU27FqitXqu45Vo6VGp5aCHf6M" src="https://vercel.live/_next-live/feedback/feedback.js"></script>