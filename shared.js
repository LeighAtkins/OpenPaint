...
class SharedProjectViewer {
  ...
  async loadSharedProject() {
    try {
      ...
      console.log('Vector strokes:', this.projectData.strokes);
      console.log('Line strokes:', this.projectData.strokeSequence);
      console.log('Image scale:', this.projectData.imageScales);
      console.log('Image position:', this.projectData.imagePositions);
    ...
    try {
      ...
      const drawWith = img => {
        ...
        const offscreen =
          ...
          imageRect.x > this.canvas.width ||
          imageRect.y > this.canvas.height;
        if (offscreen) {
      ...
      if (!cached) {
        ...
        this.drawStrokes(
          ...
          (this.canvas.width -
            ...
            position.x,
          (this.canvas.height -
            (this.projectData.originalImageDimensions?.[imageLabel]?.height || 0) * scale) /
            2 +
            position.y
        );
      }
    ...
    if (strokeData.type === 'straight' || strokeData.type === 'arrow') {
      ...
      if (strokeData.points.length >= 2) {
        ...
        const end = this.transformPoint(
          strokeData.points[strokeData.points.length - 1],
          scale,
          offsetX,
          offsetY
        );
  ...
  generateMeasurementForm() {
    ...
    if (strokes.length === 0) {
      formContainer.innerHTML =
        '<p style="text-align: center; color: #666;">No measurements found in this project.</p>';
      return;
    }
    ...
    inputs.forEach(input => {
      input.addEventListener('input', () => {
        ...
        this.projectData.strokeMeasurements[imageLabel] =
          this.projectData.strokeMeasurements[imageLabel] || {};
        this.projectData.strokeMeasurements[imageLabel][strokeLabel] = {
          value,
  ...
  onCanvasMouseMove(evt) {
    ...
    this.projectData.customLabelPositions[imageLabel] =
      this.projectData.customLabelPositions[imageLabel] || {};
  ...
  async submitMeasurements() {
    try {
      ...
      this.showSubmitResult(
        'success',
        'Measurements submitted successfully! Thank you for your input.'
      );
