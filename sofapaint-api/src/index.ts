...
export interface Env {
	...
	GUIDE_API_KEY?: string;
	MEASUREMENT_GUIDES?: R2Bucket;
}
...
export default {
	async fetch(req: Request, env: Env): Promise<Response> {
		...
		const apiKey = (env.GUIDE_API_KEY || 'dev-secret').trim();
		...
		if (req.method === 'GET' && url.pathname === '/measurement-guides/svg') {
			const code = String(url.searchParams.get('code') || '')
				.trim()
				.toUpperCase();
			const viewRaw = String(url.searchParams.get('view') || '')
				.trim()
				.toLowerCase();
		...
		if (req.method === 'POST' && url.pathname === '/remove-background') {
			try {
				...
				const transformed = await env.IMAGES.input(src.body)
